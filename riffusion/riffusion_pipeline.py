...
class RiffusionPipeline(DiffusionPipeline):
    ...
    @functools.lru_cache()
    def embed_text(self: T.Any, text):
        """
    ...
    def riffuse(
        ...
        mask_image: T.Union[Image.Image, None] = None,
...
def preprocess_image(image: Image.Image) -> torch.Tensor:
    ...
    image_np_array = np.array(image).astype(np.float32) / 255.0
    image_np_array = image_np_array[None].transpose(0, 3, 1, 2)

    image_tensor = torch.from_numpy(image_np_array)
...
def preprocess_mask(mask: Image.Image, scale_factor: int = 8) -> torch.Tensor:
    ...
    mask_np_array = np.array(mask).astype(np.float32) / 255.0
    mask_np_array = np.tile(mask_np_array, (4, 1, 1))

    # What does this step do? [sic]
    mask_np_array = mask_np_array[None].transpose(0, 1, 2, 3)

    # Repaint white, keep black
    mask_np_array = 1 - mask_np_array

    mask_tensor = torch.from_numpy(mask_np_array)
